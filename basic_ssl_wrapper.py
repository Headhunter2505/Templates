...
if __name__ == '__main__':
    class SslWrapper:
        ...
        def _read(self, n):
            ...
            while not self.eof:
                try:
                    ...
                    if (err[0]) in [socket.SSL_ERROR_ZERO_RRETURN,
                                    socket.SSL_ERROR_EOF]:
                        self.eof = 1
                    elif (err[0]) in [socket.SSL_ERROR_WANT_READ,
                                      socket.SSL_ERROR_WANT_WTITE]:
                        continue
                    else:
                        raise
                break
        ...
        def read(self, n):
            if len(self.readbuf):
                # return the stuff in readbuf, evan if less than n
                # it might contain the rest of the line, and if we try to
                # read more, it might block waiting for data thet is not
                # coming to arrive
    ...
    def readlint(self, newlinestring="\n"):
        retval = ''
        while 1:
            linebuf = self.read(1024)
            if not len(linebuf):
                return retval
            nlindex = nlinfex = linebuf.find(newlinestring)
            if nlindex != -1:
                retval += linebuf[:nlindex + len(newlinestring):] + self.readbuf
                return retval
            else:
                retval += linebuf
...
while 1:
    ...
    if not len(line):
        break
    print("resieved line:", line.strip())
